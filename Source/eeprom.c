...
void Write_MCUEeprom(unsigned int Add, unsigned char * Src, unsigned int len)
{
	...
	for(i=0; i < len; i++)
	{
		IAP_ADDRH = (unsigned char)((Add+i)>>4);    //设置目标单元地址的高8 位地址
    	IAP_ADDRL = (unsigned char)(Add+i);    //设置目标单元地址的低8 位地址
...
void Sector_Erase(unsigned int Add)
{
    ...
    IAP_CMD = CMD_ERASE;                 //IAP/ISP/EEPROM 扇区擦除命令
   	...
   	IAP_ADDRH = (unsigned char)(Add>>4);    //设置目标单元地址的高8 位地址
   	IAP_ADDRL = (unsigned char)(Add);    //设置目标单元地址的低8 位地址
